...
class Master(object):
  def __init__(self, game, slaves, slaves_output_dir=None, game_log=None, expected_turns=0):
    ...
    self.game = game
    self.slaves = {game.get_country(country): Slave(module_paths['tactical'], module_paths['strategic'], output_location=os.path.join(slaves_output_dir, country) if slaves_output_dir else None) for country, module_paths in slaves.items()}
    if game_log is None:
      self.game_log = None
    else:
  ...
  def log_turn(self, turn_commands):
    ...
    turn_dict = {
      ...
      'commands': {country.name: commands for country, commands in turn_commands.items()},
    }
